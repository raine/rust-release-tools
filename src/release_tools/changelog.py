...
#      update-changelog --pending v0.1.50
#
# Workflows:
#   Batch mode (main):
...
def backfill_tags() -> None:
    """Find missing tags and generate changelog entries for them."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Update CHANGELOG.md from git tags using AI"
    )
    parser.add_argument(
        "--pending",
        metavar="VERSION",
        help="Generate entry for a pending release (e.g., v0.1.50)",
    )
    args = parser.parse_args()

    if args.pending:
        generate_for_pending(args.pending)
    else:
        backfill_tags()
